...
src/
  ...
  assets/
    ...
    beroun.jpg, zahorany.jpeg – fotky provozoven pro Locations
    gallery/                  – ~79 fotek z provozu (gallery-01.jpg…gallery-79.jpg,
                                zmenšeno na max. 1300 px přes sips), Gallery.jsx je
                                načítá automaticky přes `import.meta.glob`, stránkováno po 12
public/
...
- **Domažlice (Zahořany)** – Zahořany 3, 344 01 Domažlice – Po, Pá
...
1. Zmenši fotku (doporučeno delší hrana ~1300 px, např. `sips -Z 1300 in.jpg --out out.jpg`)
2. Ulož ji do `src/assets/gallery/` s dalším číslem v řadě, např. `gallery-80.jpg`
3. Gallery.jsx ji automaticky načte přes `import.meta.glob` – žádný ruční import není potřeba.
   Volitelně přidej alt text do pole `altTexts` v `Gallery.jsx` (podle pořadí souborů),
   jinak se použije generický fallback "Fotka z LEN studia".

### Vyměnit fotku v Hero / About
V `Hero.jsx` (`heroPhoto`) nebo `About.jsx` (`aboutPhoto`) over import a nahraď soubor v `src/assets/`.

### Fotky u provozoven (Locations)
`src/components/Locations.jsx` mapuje fotky na provozovny přes objekt `photos` (klíč = `loc.key`
z `contact.js`). Pro výměnu stačí nahradit soubor `src/assets/beroun.jpg` / `zahorany.jpeg`.
